<?php
	...
	function checkFilms($title){
		...
		$query = "SELECT * FROM `uploaded_films` WHERE `film_title` = '$title'";
	...
	function insertFilm($title, $desc, $filename){
		include '../reg_log/db.php';
		$query = "INSERT INTO uploaded_films (film_id, film_title, film_description, filename, date_added) VALUES ('','$title', '$desc', '$filename', NOW())";
		$result = mysqli_query($conn, $query);
