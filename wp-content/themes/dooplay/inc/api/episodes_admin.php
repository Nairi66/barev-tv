		...
		if (($_GET["se"] != NULL) && ($_GET["te"] != NULL)) {
			...
			if (($ids != NULL) && ($dtemporada != NULL)) {
				...
				for ($cont = 1; $cont <= $sdasd; $cont++) {
					...
					wp_set_post_terms($post_id, $castor, 'dtgueststar', false);
					wp_set_post_terms($post_id, $director, 'dtdirector', false);
			...
			wp_redirect(get_admin_url() . "edit.php?post_type=episodes");
			exit;
		}
		else {
			echo 'error';
