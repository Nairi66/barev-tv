...
define('DT_RENOVAR', '');
define('DT_SUPPORT_FORUMS', '');
define('DT_PAGE_THEME', '');
define('DT_CHANGELOG', '');
define('DT_DOC', '');
...
{  
     ...
     if(1 != $pages)
     {
         echo "<div class=\"pagination\"><span>". __('Page','mtms') ." ".$paged." " . __('of','mtms') . " ".$pages."</span>";
...
if(is_admin() and current_user_can('administrator')){
	...
	// Page favorites
	$page_favorites = get_option('dt_favorites_page');
	if(empty($page_favorites)){
		$post_id = wp_insert_post(array(		
		  'post_content'   => '',
		  'post_name'      => __('My favorites','mtms'),
		  'post_title'     => __('My favorites','mtms'),
		  'post_status'    => 'publish',
		  'post_type'      => 'page',
		  'ping_status'    => 'closed',
		  'post_date'      => date('Y-m-d H:i:s'),
		  'post_date_gmt'  => date('Y-m-d H:i:s'),
		  'comment_status' => 'closed',
		  'page_template'  => 'pages/favorites.php'
		));
		$get_04 = get_option('siteurl').'/' . sanitize_title(__('My favorites','mtms')).'/';
		update_option('dt_favorites_page', $get_04);
	}
	// Page contact
