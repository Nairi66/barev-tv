...
<h3><?php _e('Submit a link','mtms'); ?></h3>
	<form id="postlink">
		...
		<a href="#" onclick="document.getElementById('captcha').src = '<?php echo get_template_directory_uri(); ?>/inc/captcha.php?' + Math.random();document.getElementById('captcha_code').value = '';return false;"><i class="icon-refresh2"></i></a>
...
$(document).ready(function(){
    $('#postlink').submit(function(){
        ...
        $.ajax({
            type: 'POST',
			cache: false,
            url: '<?php echo get_template_directory_uri(); ?>/inc/api/links.php', 
            data: $(this).serialize()
        })
        .done(function(html){
			// Agregando clase
